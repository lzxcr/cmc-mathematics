...
使用定理应核实假设，较深结论标明引用并提供专业阅读方向。点态展开不能冒充一致展开；交换求和、积分、极限或最值前，要解释余项如何控制。涉及题面差异时，区分转录校对、术语约定与解答补全，并说明本书采用的表述；未经可靠原卷核对，不把整理材料中的歧义归为命题错误。编译成功、数值试验和 AI 解释均不能替代证明。
